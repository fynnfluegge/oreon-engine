package org.oreon.demo.gl.oreonworlds2.assets.rocks;
...
import org.oreon.demo.gl.oreonworlds2.shaders.InstancingGridShader;
import org.oreon.demo.gl.oreonworlds2.shaders.assets.rocks.RockHighPolyShader;
import org.oreon.modules.gl.terrain.Terrain;

public class Rock01Cluster extends InstancingCluster{
	
	public Rock01Cluster(int instances, Vec3f pos, List<InstancedDataObject> objects) {
	...
	public void render(){
		if (getCenter().sub(CoreSystem.getInstance().getScenegraph().getCamera().getPosition()).length() < 800){
			super.render();
		}
	}
