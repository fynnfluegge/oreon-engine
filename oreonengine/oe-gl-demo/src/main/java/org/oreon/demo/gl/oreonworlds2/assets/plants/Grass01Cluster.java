...
import org.oreon.demo.gl.oreonworlds2.shaders.assets.plants.GrassShader;
...
//			object.addComponent(Constants.SHADOW_RENDERER, shadowRenderer);
	...
	{	
		...
		if (CoreSystem.getInstance().getRenderingEngine().isGrid()){
			...
			((Renderer) ((GameObject) getChildren().get(0)).getComponents().get("Renderer")).getRenderInfo().setShader(GrassShader.getInstance());
		}
	}
}
