...
void main(void){
		...
		&& gl_GlobalInvocationID.y < windowHeight-4)
	{
		if (linDepth > 0.06) {
			...
			for (int i=0; i<9; i++){
				if (linearize(texelFetch(depthmap, computeCoord + ivec2(0,((i-4)*1.4)),0).r) > 0.059)
			...
			for (int i=0; i<9; i++){
				if (linearize(texelFetch(depthmap, computeCoord + ivec2(0,(i-4)),0).r) > 0.039)
			...
			for (int i=0; i<7; i++){
				if (linearize(texelFetch(depthmap, computeCoord + ivec2(0,(i-3)),0).r) > 0.019)
