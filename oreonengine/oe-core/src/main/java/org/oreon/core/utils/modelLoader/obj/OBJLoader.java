...
public class OBJLoader {
	...
	public Model[] load(String path, String objFile, String mtlFile)
	{
		long time = System.currentTimeMillis();
			...
			if (mtlFile != null){
				try{
						InputStream is = OBJLoader.class.getClassLoader().getResourceAsStream(path + "/" + mtlFile);
						mtlReader = new BufferedReader(new InputStreamReader(is));
						String line;
						String currentMtl = "";
			...
			// load .obj
			try{
				InputStream is = OBJLoader.class.getClassLoader().getResourceAsStream(path + "/" + objFile);
