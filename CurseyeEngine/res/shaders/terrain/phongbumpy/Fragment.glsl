...
{		
	...
	float rockgrassBlending = texture(splatmap, mapCoords).b;
	...
	if (highDetailF ==  1)
	{
		...
		vec3 rockgrassNRM = normalize(2*(texture(rockgrass.normalmap, texCoordF).rbg)-1);
		...
		vec3 bumpNormal = normalize(rockgrassBlending * rockgrassNRM + rockBlending * rockNRM + snowBlending * snowNRM);
	...
	emission = rockgrassBlending * rockgrass.emission + rockBlending * rock.emission + snowBlending * snow.emission;
	shininess = rockgrassBlending * rockgrass.shininess + rockBlending * rock.shininess + snowBlending * snow.shininess;
	...
	vec3 rockgrassTexel = texture(rockgrass.texture_, texCoordF).xyz;
	vec3 rockTexel = texture(rock.texture_, texCoordF).xyz;
	vec3 snowTexel = texture(snow.texture_, texCoordF).xyz;
	
	diffuseLight  += vec3(sunlight.color) * diffuseFactor;
	specularLight += vec3(sunlight.color) * specularFactor;
	
	vec3 fragColor = (min(vec3(1.0),rockgrassTexel * rockgrassBlending + rockTexel * rockBlending + snowTexel * snowBlending)  * diffuseLight + specularLight) * sunlight.ambient * ambientOcclusion;
