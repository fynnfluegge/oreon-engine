...
layout (binding = 1, rgba32f) uniform image2D Dx;
...
void main(void)
{
	...
	vec2 x = ivec2(gl_GlobalInvocationID.xy);// - N/2.0;
	...
	complex h_k_t_dx = mul(dx, h_k_t);
	
	// dz
	complex dy = complex(0.0,-k.y/magnitude);
	complex h_k_t_dz = mul(dy, h_k_t);
		
	imageStore(Dy, ivec2(gl_GlobalInvocationID.xy), vec4(h_k_t.real, h_k_t.im, 0, 1));
	imageStore(Dx, ivec2(gl_GlobalInvocationID.xy), vec4(h_k_t_dx.real, h_k_t_dx.im, 0, 1));
	imageStore(Dz, ivec2(gl_GlobalInvocationID.xy), vec4(h_k_t_dz.real, h_k_t_dz.im, 0, 1));
}