...
public class TerrainPhongBumpy extends Shader{
	
	private static TerrainPhongBumpy instance = null;
	
	public static TerrainPhongBumpy getInstance() 
	{
	    if(instance == null) 
	    {
	    	instance = new TerrainPhongBumpy();
	    }
	      return instance;
	}
	
	protected TerrainPhongBumpy()
	{
		super();

		addVertexShader(ResourceLoader.loadShader("terrain/phongbumpy/Vertex.glsl"));
		addTessellationControlShader(ResourceLoader.loadShader("terrain/phongbumpy/Tessellation Control.glsl"));
		addTessellationEvaluationShader(ResourceLoader.loadShader("terrain/phongbumpy/Tessellation Evaluation.glsl"));
		addGeometryShader(ResourceLoader.loadShader("terrain/phongbumpy/Geometry.glsl"));
		addFragmentShader(ResourceLoader.loadShader("terrain/phongbumpy/Fragment.glsl"));
		compileShader();
		...
		addUniform("rockgrass.texture_");
		addUniform("rockgrass.normalmap");
		addUniform("rockgrass.displacemap");
		addUniform("rockgrass.diffusemap");
		addUniform("rockgrass.displaceScale");
		addUniform("rockgrass.shininess");
		addUniform("rockgrass.emission");
		addUniform("rock.texture_");
	...
	{	
		...
		setUniformi("rockgrass.texture_", 4);
		glActiveTexture(GL_TEXTURE5);
		terrain.getMaterial1().getNormalmap().bind();
		setUniformi("rockgrass.normalmap", 5);
		glActiveTexture(GL_TEXTURE6);
		terrain.getMaterial1().getDisplacemap().bind();
		setUniformi("rockgrass.displacemap", 6);
		
		setUniformf("rockgrass.displaceScale", terrain.getMaterial1().getDisplaceScale());
		setUniformf("rockgrass.shininess", terrain.getMaterial1().getShininess());
		setUniformf("rockgrass.emission", terrain.getMaterial1().getEmission());
		
		glActiveTexture(GL_TEXTURE8);
		terrain.getMaterial2().getDiffusemap().bind();
		setUniformi("rock.texture_", 8);
