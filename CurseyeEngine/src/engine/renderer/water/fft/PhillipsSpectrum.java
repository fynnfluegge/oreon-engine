...
import engine.gpgpu.fastFourierTransform.FourierSpectrum;
import engine.math.Vec2f;
import engine.shaderprograms.water.fft.PhillipsSpectrumShader;

public class PhillipsSpectrum extends FourierSpectrum{
	
	private Vec2f w = new Vec2f(1,0).normalize();
	private float g = 9.81f;
	private float velocity = 28;
	...
	public PhillipsSpectrum(int N, int L)
	{
		...
		setShader(PhillipsSpectrumShader.getInstance());
	...
	public void render() {
		...
		getShader().execute();
		getShader().sendUniforms(getN(), L, A, w, l);
