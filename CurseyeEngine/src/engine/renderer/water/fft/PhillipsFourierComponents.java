package engine.renderer.water.fft;
...
import engine.gpgpu.fastFourierTransform.FourierComponents;
import engine.shaderprograms.water.fft.PhillipsFourierComponentsShader;

public class PhillipsFourierComponents extends FourierComponents{
	
	private Texture dyComponents;
	private Texture dxComponents;
	private Texture dzComponents;

	public PhillipsFourierComponents(int N, int L) {
		
		super(N, L);
		setSpectrum(new PhillipsSpectrum(N,L));
		setShader(PhillipsFourierComponentsShader.getInstance());
	...
	public void update(float t) {
		...
		glBindImageTexture(3, ((PhillipsSpectrum)getSpectrum()).geth0k().getId(), 0, false, 0, GL_READ_WRITE, GL_RGBA32F);
		glBindImageTexture(4,  ((PhillipsSpectrum)getSpectrum()).geth0kminus().getId(), 0, false, 0, GL_READ_WRITE, GL_RGBA32F);
