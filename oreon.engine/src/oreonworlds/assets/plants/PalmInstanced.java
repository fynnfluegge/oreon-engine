...
import oreonworlds.shaders.plants.PalmShadowShader;
import oreonworlds.shaders.plants.PalmBillboardShader;
...
public class PalmInstanced extends Node{
	...
	public void update()
	{	
		if (center.sub(Camera.getInstance().getPosition()).length() < 550){
...
//			System.out.println(highPolyModelTransforms.size());
	...
	public void updateUBOs(){
		...
		for (TransformsInstanced transform : transforms){
			if (transform.getTranslation().sub(Camera.getInstance().getPosition()).length() > 150){
