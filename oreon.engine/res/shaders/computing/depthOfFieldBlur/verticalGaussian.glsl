...
void main(void){
		...
		&& gl_GlobalInvocationID.y < windowHeight-4)
	{
		...
		if (linDepth > 0.05) {
			...
			color = vec3(0,0,0);
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,-3)).rgb * gaussianKernel7[0];
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,-2)).rgb * gaussianKernel7[1]; 
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,-1)).rgb * gaussianKernel7[2]; 
			color += imageLoad(horizontalBlurSceneSampler, computeCoord).rgb * gaussianKernel7[3]; 
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,1)).rgb * gaussianKernel7[4]; 
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,2)).rgb * gaussianKernel7[5];
			color += imageLoad(horizontalBlurSceneSampler, computeCoord + ivec2(0,3)).rgb * gaussianKernel7[6];
		}
	}
